namespace PpeManager.Api.Application.DTO
{
    public record WorkerDTO
    {
        ...
        {
            return new WorkerDTO(worker.Id, worker.Name.ToString(), worker.Cpf.ToString(), worker.Role, worker.RegistrationNumber, worker.AdmissionDate.ToString(), CompanyDTO.FromEntity(worker.Company) , worker.Ppes.Select(p => PpeWithoutCertificationsDTO.FromEntity(p)).ToList(), worker.PpePossessions.Select(p => PpePossessionDTO.FromEntity(p)).ToList());
        }
